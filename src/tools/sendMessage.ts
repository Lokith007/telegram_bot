...
export const sendMessage = {
  ...
  async run(args: z.infer<typeof SendMessageSchema>) {
    try {
      ...
      const token = validatedArgs.token || process.env.TELEGRAM_BOT_TOKEN;
      if (!token) {
        throw new Error("Telegram bot token is required. Please provide it as an argument or set TELEGRAM_BOT_TOKEN environment variable.");
      }
